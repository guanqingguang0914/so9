    ...
    public void receiveClientData() {
        while (isBrainInfoReceive) {
            try {
                ...
                if (data != null && (isActive  || data[5] == (byte)0x20 && data[6] == (byte)0x13)) {
            ...
            try {
                ...
                while (isBrainInfoSend) {
                    ...
                    if (data != null) {
                        ...
                        DatagramPacket target = new DatagramPacket(data, 0, data.length, infoDatagraPacket1.getAddress(), infoDatagraPacket1.getPort());
                        infoDatagramSocket.send(target);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private void receiveM(byte[] data) {
        try {
            ...
            //jingh add RTSP视频传输
                ...
                MathingM.sendWindowM(data);
                //收到pad端消息透传至我叫奥科流思2.0
            } else if (data[5] == (byte) 0x20 && data[6] == (byte) 0x13) {
                LogMgr.d(" == receiveM 收到pad端消息透传至我叫奥科流思2.0 == ");
                MSender.getInstance().handActionForBean(MUtils.M_SEND_PROTOCOL , "" ,data);
            } if (data[5] == (byte) 0x20 && data[6] == (byte) 0x14) {
                //获取图灵注册结果
                boolean regResult = SharedPreferenceTools.getBoolean(
                        Application.getInstance(),
                        SharedPreferenceTools.KEY_TURING_REGISTER_STATE,
                        false);
                int contentLength = (MUtils.errMsg == null) ? 0 : MUtils.errMsg.length();
                int errCode = regResult ? MUtils.TULING_INIT_SUCCES : MUtils.errCode;
                errCode = MUtils.errCode;
                byte[] cmdBytes = new byte[1 + 2 + contentLength];
                cmdBytes[0] = 0x02;
                cmdBytes[1] = (byte) (errCode >> 8 & 0xFF);
                cmdBytes[2] = (byte) (errCode & 0xFF);
                if (contentLength > 0)
                System.arraycopy(MUtils.errMsg.getBytes("UTF-8"), 0, cmdBytes, 3, contentLength);
                byte[] sendData = ProtocolUtil.buildProtocol(
                        (byte) GlobalConfig.BRAIN_TYPE,
                        GlobalConfig.KNOW_ROBOT_OUT_CMD_1,
                        GlobalConfig.KNOW_ROBOT_SN_OUT_CMD_2,
                        cmdBytes);
                LogMgr.d(TAG, "图灵注册结果：" + Utils.bytesToString(sendData));
...
//            }
    ...
    private void handleAbilixChartCmd(byte[] data) {
        // 此处的fileName 的格式为 文件夹名\动作文件名&音频文件名
        String fileName = BrainUtils.getFileNameFromCmd(data);
        LogMgr.d(TAG, "fileName = " + fileName);
        String filePath = fileName.split("\\\\")[0]; // filePath为文件夹名
        LogMgr.d(TAG, "filePath = " + filePath);
        File file = new File(BrainUtils.ABILIX + BrainUtils.ABILIX_CHART, filePath);
        if (file.exists()) {
            LogMgr.d(TAG, "receiveAll() isAbilixChartCmd 文件存在 fileName = "
                    + filePath);
            byte[] downData = new byte[data.length - 12];
            System.arraycopy(data, 11, downData, 0, downData.length);
            BrainActivity.getmBrainActivity().exeFileFunc(GlobalConfig.APP_TYPE_ABILIX_CHART, fileName, 1);
        } else {
            LogMgr.d(TAG, "receiveAll() isAbilixChartCmd 文件不存在 fileName = "
                    + filePath);
            // 文件不存在时，给客户端发送回应 然后开始传输文件
            byte[] tempdata = new byte[5];
            tempdata[0] = (byte) 0x01;
    ...
    public void filterCmdProcess(byte[] data) {
        try {
            ...
            }  else if (BrainUtils.isAbilixChartCmd(data) && BrainUtils.isSkillPlayerPlayCmd(data)) {
                LogMgr.d("AbilixChart播放命令");
                handleAbilixChartCmd(data);
