...
import com.abilix.brain.m.MSender;
import com.abilix.brain.m.MUIChanger;
 ...
 */
public class BrainActivity extends TypefaceBaseActivity {
     ...
     * 我叫奥克流思的包名
     */
    public final static String OCULUSNAME = "com.abilix.learn.oculus.imoculus";

    public final static String OCULUSNAME_OLD = "com.abilix.oculus";
    /**
     * BrainActivity唯一实例
     */
    ...
    private boolean executeCurrentPageFlag = false;
    /**
     * 获取BrainActivity唯一实例
     */
    ...
    public Handler mBrainActivityHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            LogMgr.d("handleMessage: " + msg.what);
            switch (msg.what) {
                //case 0x01:
                //    break;
                //case 0x02:
...
//	private TextView mTextForVjc;
    ...
    //修改开机引导设置语言成功后APP会重新调用onCreate,导致GiftView语言设置完成到WiFi的切换出现空隙可以点击Brain程序
    private static boolean mGuideDrawWiFiIconFlag = false;
    //开机引导隐藏语言设置选项
    private static boolean mGuideHideLangItem = true;
...
//        }
        ...
        if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_M) {
            ...
            if (!Application.getInstance().isFirstStartApplication()
                ...
                if (MUtils.sIsVoiceAppBack) {
                    MUIChanger.getInstance().changeMGifState(mMyGiftView, MUIChanger.MGifState.M_STATE_HEART_JUMP);
    ...
    protected void onResume() {
        ...
        //M系列选中二维码不需要进行情绪倒计时
        if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_M) {
            if (MUtils.isSpeakAndBindingFinish() && !MUtils.isBrainFirstStart()) {
                if (executeCurrentPageFlag) {
                    executeCurrentPageFlag = false;
                    if (mCurrentPageAppInfo.getPageType() == AppInfo.PAGE_TYPE_QR_CODE || MUtils.sIsProgram) {
                        MUIChanger.getInstance().stopAD();
                    } else {
                        MUIChanger.getInstance().startAD();
                    }
                }
                //
                if (MUtils.sIsYuyinControl) {
    ...
    protected void onPause() {
        ...
        if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_M) {
            //进入其他应用或弹窗时停止timer;
            MUIChanger.getInstance().stopAdOnly();
        }
    }
...
//        mConnectBroadcastReceiver = null;
    ...
    protected void onDestroy() {
        ...
        if(GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_M){
            if(mGuideBroadcastReceiver != null){
                unregisterReceiver(mGuideBroadcastReceiver);
            }
            mGuideBroadcastReceiver = null;
            //退出情绪模式，发送B_GO_TO_APP，停止传感器感应
            MUIChanger.getInstance().stopEmotion();
    ...
    private void initViewM() {
        ...
        mViewPager.addOnPageChangeListener(new MyViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int arg0) {
                ...
                LogMgr.i("当前选定页面序号 = " + arg0 + " 页面名 = " + mCurrentPageAppInfo.getName()
                        + " mAppInfos.size() = " + mAppInfos.size() + " mCurrentPageNum%mAppInfos.size() = "
                        + mCurrentPageNum % mAppInfos.size());

                //M系列选中二维码不需要进行情绪倒计时
                if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_M && isForegroundFirst) {
                    if (MUtils.isSpeakAndBindingFinish() && !MUtils.isBrainFirstStart()) {
                        if (mCurrentPageAppInfo.getPageType() == AppInfo.PAGE_TYPE_QR_CODE || MUtils.sIsProgram) {
                            MUIChanger.getInstance().stopAD();
                        } else {
...
//                        mImageView.setVisibility(View.GONE);
        ...
        if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_M ) {
            ...
            mMyGiftView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    MUIChanger.getInstance().stopEmotion();
                    LogMgr.d("FZXX", "当前页面是 " + mCurrentPageAppInfo.getName());
                    if (mCurrentPageAppInfo.getPageType() != AppInfo.PAGE_TYPE_QR_CODE && !MUtils.sIsProgram) {
                        MUIChanger.getInstance().startAD();
                    }
                }
            });
            if (MUtils.isBrainFirstStart()) {
                if (mGuideDrawWiFiIconFlag) {
                    mGuideDrawWiFiIconFlag = false;
                    MUIChanger.getInstance().changeMPicState(mMyGiftView, MUIChanger.MPicState.M_PIC_STATE_WIFI);
                } else {
                    /*if (mGuideHideLangItem) {
                        MUIChanger.getInstance().changeMPicState(mMyGiftView, MUIChanger.MPicState.M_PIC_STATE_WIFI);
                    } else {
                        MUIChanger.getInstance().changeMPicState(mMyGiftView, MUIChanger.MPicState.M_PIC_STATE_LANGUAGE);
                    }*/
                    MUIChanger.getInstance().changeMGifState(mMyGiftView, MUIChanger.MGifState.M_STATE_START);
                }
            } else {
                MUIChanger.getInstance().changeMGifState(mMyGiftView, MUIChanger.MGifState.M_STATE_HEART_JUMP);
            }
        ...
        for (int i = 0; i < 7; i++) {
            ...
            switch (i) {
                ...
                case 2:
                    ...
                    appInfo.setPkgName(GlobalConfig.APP_PKGNAME_INSTANT_MESSAGING);
                    break;
                case 3:
                    appInfo.setPageType(AppInfo.PAGE_TYPE_M_ABILITY_TRAINING);
                    appInfo.setName(getString(R.string.m_training_instruction_learing));
        ...
        if (mAppInfos != null) {
            for (ResolveInfo reInfo : resolveInfos) {
                ...
                if (TextUtils.equals(GlobalConfig.APP_PKGNAME_BRAINSET, pkgName)) {
                    ...
                    } else if (GlobalConfig.BRAIN_CHILD_TYPE == GlobalConfig.ROBOT_TYPE_K204) {
                        mAppInfos.add(1, appInfos);
                    } else {
            ...
            switch (i) {
                ...
                case 1:
                    if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_AF || GlobalConfig.BRAIN_CHILD_TYPE == GlobalConfig.ROBOT_TYPE_CU
                            || GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_H3 || GlobalConfig.BRAIN_CHILD_TYPE == GlobalConfig.ROBOT_TYPE_C9
                            || GlobalConfig.BRAIN_CHILD_TYPE == GlobalConfig.ROBOT_TYPE_K204) {
                        continue;
                    }
                    ...
                    break;
                case 2:
                    if (GlobalConfig.BRAIN_CHILD_TYPE == GlobalConfig.ROBOT_TYPE_K204) {
                        continue;
                    }
                    appInfo.setPageType(AppInfo.PAGE_TYPE_QR_CODE);
                    appInfo.setName("二维码");
...
//        }
        ...
        for (int i = 0; i < mAppInfos.size(); i++) {
            if (mAppInfos.get(i).getPageType() == AppInfo.PAGE_TYPE_QR_CODE) { // 生成二维码界面
                ...
                if (mAppInfos.get(i).getPageType() == AppInfo.PAGE_TYPE_APK) {
                    ...
                    if (mAppInfos.get(i).getFileType() == AppInfo.FILE_TYPE_SKILLPLAYER) {
                        imageView.setImageResource(R.drawable.m_skill);
                    } else {
                        imageView.setImageResource(R.drawable.m_chart);
                    }
                }
    ...
    private void initViewFunc(final int i, final ViewHoder viewHoder, final AppInfo appInfo) {
        ...
        //M系列一級頁面取消长按事件
        if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_M && appInfo.getPageType() != AppInfo.PAGE_TYPE_FILE) {
            return;
        }
    ...
    private void insertPage(int i, AppInfo appInfo) {
        ...
        if (GlobalConfig.BRAIN_CHILD_TYPE == GlobalConfig.ROBOT_TYPE_CU
            ...
            mAppInfos.add(i, appInfo);
                ...
                && appInfo.getPageType() == AppInfo.PAGE_TYPE_FILE) {
            View childview = mLayoutInflater.inflate(R.layout.brain_content_layout, null);
            childview.setBackgroundResource(R.drawable.m_bg);
            ViewHoder vh = new ViewHoder();
            TextView textview = (TextView) childview.findViewById(R.id.textview);
...
//        mViewHoders.add(i, viewHoder);
        ...
        if (i == 0) {
            ...
            if (GlobalConfig.BRAIN_CHILD_TYPE == GlobalConfig.ROBOT_TYPE_CU) {
                ...
                textView.setVisibility(View.GONE);
                qr_iv.setBackgroundColor(Color.TRANSPARENT);
                qr_iv.setImageResource(R.drawable.m_programming);
...
//                        appInfo.setName(f.getName().trim());//修复Bug #17797 【1.1.1.18】下载ProgramA后，发现机器人端出现2个同名项目
    ...
    private void queryAppInfo() {
        ...
        if (mAppInfos != null) {
            for (ResolveInfo reInfo : resolveInfos) {
                ...
                //xiongxin@20180108 add start for k9 用户自检的位置插入设置之后
                }else if(GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_C9 &&
                        TextUtils.equals("com.abilix.k9_customer_factory", pkgName)){
                    appInfos.setOrder(AppInfo.PAGE_ORDER_K9_CUSTOM_FACTORY);
    ...
    private void executeCurrPageFuncForM(AppInfo appInfo) {
        if (!MUtils.sIsBootspeakingFinish) {
            LogMgr.e("executeCurrPageFunc() sIsBootspeakingFinish is false");
            return;
        }
        if (null == appInfo) {
        ...
        try {
            ...
            MUtils.sIsYuyinControl = false;
            MUIChanger.getInstance().stopAD();
            if (appInfo.getPageType() != AppInfo.PAGE_TYPE_M_AKLS
                    && appInfo.getPageType() != AppInfo.PAGE_TYPE_PROGRAM) {
                //点击页面执行程序是停止进入情绪计时
                //MUIChanger.getInstance().stopEmotion();
                //执行brain界面程序时发送B_GO_TO_APP，停止传感器感应
                LogMgr.d("FZXX", "handAction(MUtils.B_GO_TO_APP)");
                MSender.getInstance().handActionForBean(MUtils.B_GO_TO_APP);
            }
            //编程
            if (appInfo.getPageType() == AppInfo.PAGE_TYPE_PROGRAM) {
                ...
                stopAnimaAndFunc(false);
                //未来通信站
            }else if (appInfo.getPageType() == AppInfo.PAGE_TYPE_M_FUTURE_COMMUNICATION) {
                ...
                Utils.openApp(BrainActivity.this, GlobalConfig.APP_PKGNAME_OCULUS_ORDER, null,false);
                stopAnimaAndFunc(false);
            } else if (appInfo.getPageType() == AppInfo.PAGE_TYPE_M_AKLS) {
                //Utils.openApp(BrainActivity.this, OCULUSNAME, null,false);
                stopAnimaAndFunc(false);
                //奥科流思直接進入情緒gif
                MUIChanger.getInstance().startEmotion();
            } else {//wifi模式亮绿灯,然后关闭
                for (int i = 0; i < 3; i++) {
                    if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_H) {
                        BrainService.getmBrainService().sendMessageToControl(GlobalConfig.CONTROL_CALLBACKMODE_OLD_PROTOCOL,
...
//                        null, 1, 0);
                ...
                if (wifiAp.isMyWifiHotOpen()) {//热点是否打开了，热点打开亮红灯，H3 H5 不一样
                    if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_H) {
                        BrainService.getmBrainService().sendMessageToControl(GlobalConfig.CONTROL_CALLBACKMODE_OLD_PROTOCOL,
                                ProtocolUtil.buildProtocol((byte) 0x03, (byte) 0xA3, (byte) 0x6A, new byte[]{0x04, 0x01, (byte) 0xFF, 0, 0, (byte) 0xFE, 0x00}), null, 0, 0);
                    } else if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_H3) {
                        BrainService.getmBrainService().sendMessageToControl(GlobalConfig.CONTROL_CALLBACKMODE_OLD_PROTOCOL,
...
//                    else if(GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_M){
//                        BrainService.getmBrainService().sendMessageToControl(GlobalConfig.CONTROL_CALLBACKMODE_OLD_PROTOCOL,
//                                ProtocolUtil.buildProtocol((byte)0x02,(byte) 0xA3, (byte) 0x32,new byte[]{(byte) 0xFF,0,0}), null, 0, 0);
//                    }
                } else {//wifi模式亮绿灯,然后关闭
                    for (int i = 0; i < 3; i++) {
                        if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_H) {
...
//                        (374), 2000);
    ...
    private void changeProgramStatus(boolean isBrainInfoActive) {
        if(GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_M){
            MUtils.sIsProgram = isBrainInfoActive;

            if (isBrainInfoActive) {
                MUIChanger.getInstance().stopAD();
                LogMgr.d("FZXX", "发送  B_BRAIN_ENTER_PROGRAMMING_MODE");
                MSender.getInstance().handActionForBean(MUtils.B_BRAIN_ENTER_PROGRAMMING_MODE);
            } else {
                if (isForegroundFirst && mCurrentPageAppInfo.getPageType() != AppInfo.PAGE_TYPE_QR_CODE) {
                    MUIChanger.getInstance().startAD();
                }
                LogMgr.d("FZXX", "发送  B_BRAIN_QUIT_PROGRAMMING_MODE");
                MSender.getInstance().handActionForBean(MUtils.B_BRAIN_QUIT_PROGRAMMING_MODE);
            }
        }
    ...
    public void afterTCPDisconnected() {
        ...
        if (isFaceTrackOpened) {
            setFaceTrackMode(0);
        }
    }

    public void exeFileFunc(int appType, String name, final int mode) {
        if (!isForegroundFirst) {
            LogMgr.w("Brain不在前台");
            return;
        }
        ...
        switch (appType) {
            ...
            case GlobalConfig.SKILL_PLAYER_APP_FLAG:
                ...
                filePath = BrainUtils.ABILIX + BrainUtils.ABILIX_SKILLPLAYER;
                break;
            default:
                LogMgr.w("未知类型");
                return;
        ...
        final String program = FileUtils.getFileNameNoEx(name);
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                exeFileFunc(fileType, path, mode, program);
            }
        });
...
//            filter.setPriority(1000);
        ...
        if(GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_M){
            if(mGuideBroadcastReceiver == null){
                mGuideBroadcastReceiver = new GuideBroadcastReceiver();
            }

            IntentFilter guildFilter = new IntentFilter();
            guildFilter.addAction(MUtils.S_LANGUAGE_DONE_FILTER);
            guildFilter.addAction(MUtils.S_WIFI_DONE_FILTER);
            guildFilter.addAction(MUtils.S_FACE_DONE_FILTER);
            guildFilter.addAction(MUtils.FILTER_BRAINSET_BACK);
            guildFilter.addAction(MUtils.FILTER_OPEN_LEADSET);
            registerReceiver(mGuideBroadcastReceiver, guildFilter);
        }
     ...
     * 监听M系列进入BrainSet后的状态
     */
    class GuideBroadcastReceiver extends BroadcastReceiver{

        @Override
        public void onReceive(Context context, Intent intent) {
            LogMgr.d("FZXX","intent.getAction() = " + intent.getAction());
            switch (intent.getAction()){
                case MUtils.S_LANGUAGE_DONE_FILTER:
                    MSender.getInstance().handActionForBean(MUtils.S_LANGUAGE_DONE);
                    LogMgr.d("FZXX" , "==== 发送 S_LANGUAGE_DONE ====");
                    MUtils.closeBrainSet();
                    break;
                case MUtils.S_WIFI_DONE_FILTER:
                    //TODO 等人脸识别改了换成这里
                    MSender.getInstance().handActionForBean(MUtils.S_WIFI_SUCCESS);
                    LogMgr.d("FZXX" , "=== 发送 S_WIFI_SUCCESS ===");
                    MUtils.closeBrainSet();

/*                    MSender.getInstance().handleActionEZ(MUtils.S_FACEINPUTDONE);
                    LogMgr.d("FZXX" , "=== 发送 S_FACEINPUTDONE ===");
                    MUtils.startBrainActivity(MUtils.BRAIN_PKG_NAME, MUtils.BRAIN_ACTIVITY_NAME);*/
                    break;
                case MUtils.S_FACE_DONE_FILTER:
                    MSender.getInstance().handActionForBean(MUtils.S_FACEINPUTDONE);
                    LogMgr.d("FZXX" , "=== 发送 S_FACEINPUTDONE ===");
                    MUtils.startBrainActivity(MUtils.BRAIN_PKG_NAME, MUtils.BRAIN_ACTIVITY_NAME);
                    break;

                case MUtils.FILTER_BRAINSET_BACK:
                    if (MUtils.sIsYuyinControl) {
                        MUtils.sIsYuyinControl = false;
                    } else {
                        MUIChanger.getInstance().startAD();
                ...
                case MUtils.FILTER_OPEN_LEADSET:
                    MUtils.setIsFirstStart(true);
                    break;
...
//		mRecordRelativeLayout.setText(getString(R.string.record_play_success));
    ...
    private void playWelcomeVoice() {
        ...
        if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_M) {
            ...
            String voice;
            if ((MUtils.isBrainFirstStart())) {
                voice = "oculus2_176.mp3";
            } else {
                voice = "changeddefb.mp3";
            }
            mMusicPlayer.play(voice, new OnCompletionListener() {
                @Override
                public void onCompletion(MediaPlayer mp) {
                    MUtils.sIsBootspeakingFinish = true;
                    mBrainActivityHandler.post(new Runnable() {

                        @Override
                        public void run() {
                            if (MUtils.isSpeakAndBindingFinish()) {
                                if (MUtils.isBrainFirstStart()) {//M系列:如果是第一次启动Brain
                                    MUIChanger.getInstance().setGifViewEnable(false);
                                    if (mGuideHideLangItem) {
                                        //引导页面去除语言设置界面
                                        MSender.getInstance().handActionForBean(MUtils.S_LANGUAGE_DONE);
                                    } else {
                                        MSender.getInstance().handActionForBean(MUtils.S_GUIDE);
                                        mGuideDrawWiFiIconFlag = true;
                                    }
                                } else {
                                    MUIChanger.getInstance().setGifViewEnable(true);
                                    MUIChanger.getInstance().startEmotion();
                                }
                            }
                        }
    ...
    /**
     * 设置人脸追踪
     * @param mode 0：关闭 1：打开 2：获取数据
     */
    public void setFaceTrackMode(int mode) {
        switch (mode) {
            case 0x00://0：关闭
                isFaceTrackOpened = false;
                ((UsbCamera) UsbCamera.create()).stopFaceTrackAct(BrainActivity.this);
                break;
            ...
            case 0x01://1：打开
                ...
                ((UsbCamera) UsbCamera.create()).startFaceTrackAct(BrainActivity.this, new UsbCamera.FaceTrackCallback() {
                    @Override
            ...
            case 0x02://2：获取数据
                ...
                data[0] = 0x03;
                data[1] = (byte) (faceTrackedFlag & 0xFF);
                data[2] = (byte) (posX >> 8 & 0xFF);
                data[3] = (byte) (posX & 0xFF);
                data[4] = (byte) (posY >> 8 & 0xFF);
                data[5] = (byte) (posY & 0xFF);
                byte[] returnCmd = ProtocolUtil.buildProtocol((byte) GlobalConfig.BRAIN_TYPE,
                        GlobalConfig.APP_STORE_OUT_CMD_1, GlobalConfig.APP_STORE_OUT_CMD_2_FACE_TRACK, data);
