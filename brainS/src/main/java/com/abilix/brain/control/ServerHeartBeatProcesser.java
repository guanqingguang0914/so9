...
import com.abilix.brain.utils.GetPackageInfoThread;
...
public class ServerHeartBeatProcesser {
    ...
    private static final byte PLATFORM_TYPE_PC = (byte) 0x02;
    ...
    private static final int[] PC_CONNECT_VERTION_START_RESEND_HEARTBEART = new int[]{0, 0, 0, 1};
    ...
    public void DataType(byte[] recv) {// 接收mobile数据存缓冲区
        ...
        try {
            if (recv.length >= 8 && recv[1] == GlobalConfig.APP_STORE_IN_CMD_1 && recv[2] == GlobalConfig.APP_STORE_IN_CMD_2_REQUEST_APP_INFO) {
                ...
                return;
            } else if (recv.length >= 8 && recv[1] == GlobalConfig.BRAIN_UPGRADE_IN_CMD_1 && recv[2] == GlobalConfig.BRAIN_UPGRADE_IN_CMD_2_QUERY_VERSIONS) {
                LogMgr.d("获取机器人端安装的apk信息");
                // 构成发送命令
                String json = "{data : [{\"packageName\":\"stm32\",\"verCode\":33685585},{\"packageName\":\"com.abilix.brainset\",\"verCode\":81},{\"packageName\":\"com.abilix.explainer\",\"verCode\":46},{\"packageName\":\"com.abilix.volumecontrol\",\"verCode\":8},{\"packageName\":\"com.abilix.learn.rtspserver\",\"verCode\":14},{\"packageName\":\"com.abilix.control\",\"verCode\":1316},{\"packageName\":\"com.abilix.brain\",\"verCode\":10174},{\"packageName\":\"com.abilix.updateonlinetest\",\"verCode\":139}]}";
                new GetPackageInfoThread(json).start();
                return;
            } else if (recv.length >= 8 && recv[1] == GlobalConfig.BRAIN_UPGRADE_IN_CMD_1 && recv[2] == GlobalConfig.BRAIN_UPGRADE_IN_CMD_2_INSTALL_APKS) {
                LogMgr.d("收到升级指令，开始安装apk");
                Intent launchIntent = context.getPackageManager().getLaunchIntentForPackage(GlobalConfig.APP_PKGNAME_UPDATEONLINETEST);
                if (launchIntent != null) {
                    launchIntent.putExtra("firmware_upgrade", true);
...
//            Utils.openAppForAppStore(context, pkgName);
            ...
            return;
        } else if (recv.length >= 8 && recv[1] == GlobalConfig.APP_STORE_IN_CMD_1 && recv[2] == GlobalConfig.APP_STORE_IN_CMD_2_FACE_TRACK) {
            LogMgr.i("收到人脸追踪命令 recv = "+Utils.bytesToString(recv));
            try {
                int trackMode = recv[8] & 0xFF;
                BrainActivity.getmBrainActivity().setFaceTrackMode(trackMode);
            } catch (Exception e) {
                LogMgr.e("处理人脸追踪命令 异常e = "+e);
                e.printStackTrace();
            }
            return;
...
//				LogMgr.v("当前为ios平台");
            ...
            } else if (buff[16] == PLATFORM_TYPE_PC) {
//				LogMgr.v("当前为ios平台");
                sbForLog.append(" 当前为PC平台");
...
//			LogMgr.v("M轮子电机未保护");
    ...
    public boolean isNeedToReceiveHeartbeatReply() {
        ...
        if ((platformType == PLATFORM_TYPE_IOS &&
                ...
                || (platformType == PLATFORM_TYPE_PC &&
                Utils.isLaterThanCertanCommunicationVersion(appCommunicationVersion,
                        PC_CONNECT_VERTION_START_RESEND_HEARTBEART))) {
            LogMgr.i("版本符合接受心跳回复");
            result = true;
        } else {
