...
public class Matching extends BrainInfo {
     ...
     * 定时器延迟时间
     */
    private static final int PERIOD = 30 * 1000; //ms
    /**
     * 添加一个定时器
     */
...
//                    alert_sound(context,3);
    ...
    public void playTohes(Context context, int voltage, int status, int level, int plugged, double temperature,
        ...
        if (status == BatteryManager.BATTERY_STATUS_NOT_CHARGING && level >= 10) {
...
//            entryProtectStatusH5(plugged, status);
        ...
        switch (GlobalConfig.BRAIN_TYPE) {
            ...
            case GlobalConfig.ROBOT_TYPE_M:
                if (level < 10) {
                    startTimer(context);
                    if(level <8 && (GlobalConfig.BRAIN_TYPE ==GlobalConfig.ROBOT_TYPE_M) ){
                        Intent intent = new Intent("com.abilix.closeandroidsys");
