...
//        LogMgr.i("Brain Application onCreate()1");
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_C || GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_C9) {
                    /*if (SharedPreferenceTools.getInt(context, PHOTO_DIR_CONFIG, 0) < photoDirVer) {
                        boolean result = FileUtils.deleteDirectory(FileUtils.SCRATCH_VJC_IMAGE_, false);
                        if (result) {
                            SharedPreferenceTools.saveInt(context, PHOTO_DIR_CONFIG, photoDirVer);
                        }
                    }*/
                    //对于KU系列添加图片；
                    FileUtils.saveSoundFileToSdCard(context, FileUtils.DIR_MEDIA_DEFAULT_IMAGE + File.separator, "anim_ku");
                }
        ...
        if(GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_M){
                CalligraphyConfig.initDefault(new CalligraphyConfig.Builder()
                        .setDefaultFontPath("fonts/m_fzxx.ttf")
