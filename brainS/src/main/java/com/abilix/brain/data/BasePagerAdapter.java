...
public class BasePagerAdapter extends PagerAdapter {
    ...
    @Override
    public int getCount() {
        if (!GlobalConfig.isUnlimitLoopRightSide) {
            return mAppInfos.size();
        } else if (GlobalConfig.isUnlimitLoopRightSide && !GlobalConfig.isUnlimitLoopBothSide) {
...
//			return Integer.MAX_VALUE;
    ...
    public void destroyItem(ViewGroup container, int position, Object object) {
        ...
        } else if (GlobalConfig.isUnlimitLoopBothSide && mAppInfos.size() < 4) {
            LogMgr.w("Warning：不要在这里调用removeView");
            //container.removeView(mAppInfos.get(position % mAppInfos.size()).getViewHoder().view);
        } else{
            container.removeView(mAppInfos.get(position % mAppInfos.size()).getViewHoder().view);
        }
