...
import com.abilix.brain.m.MReceiver;
...
public class BrainService extends Service {
    ...
    public static final int HANDLER_MESSAGE_H56_CAMRE_COLOR = 506;
...
//                    mHandler.sendEmptyMessageDelayed(HANDLER_MESSAGE_H34_BOOT_RECOVER, 3200);
    ...
    private DistributorBack mDistributorBack = new DistributorBack.Stub() {
        ...
        @Override
        public void disCallBack(ProtocolBean protocolbean) throws RemoteException {
            LogMgr.d("FZXX", String.format(Locale.US, "=== 接收回调 disCallBack() === orderId: %d content: %s",
                    protocolbean.getOrderId(), protocolbean.getContent()));
            Message msg = Message.obtain(mHandler);
            msg.what = WHAT_HANDLE_STATUS;
            msg.obj = protocolbean;
            msg.sendToTarget();
        }
    };
...
//        wifiAp = WifiApAdmin.getInstance(wm);
        if (GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_H && GlobalConfig.IS_USING_H_CHARGING_PROTECT) {
            startChargeTimer();
        }
    }
...
//                        sendMessageToControl(GlobalConfig.CONTROL_CALLBACKMODE_PATCH_CMD, null, null, 22, 0);//这里对应的是H3 复位
                        ...
                        mHandler.sendEmptyMessageDelayed(21,600);
...
//                    mHandler.sendEmptyMessage(319);
    ...
    public void setBrainInfoState(boolean isBrainInfoActive) {
        if (mBrainInfo != null) {
            mBrainInfo.setBrainInfoActive(isBrainInfoActive/*true*/);
        } else {
...
//                            null).setBrightnessS(msg.arg1 & 0xff);
                    ...
                    break;
                case HANDLER_MESSAGE_H56_CAMRE_COLOR://506 暂时表示H56摄像头的颜色检测。暂无实现，先注掉
//                    getColor(msg.arg1);
                    break;
                case 119:
                    //高温提醒图片弹出提示窗口
...
//                                    GlobalConfig.C_ROBOT_EN_HEITEMP);
                ...
                case WHAT_HANDLE_STATUS:
                    ProtocolBean protocolBean = (ProtocolBean) msg.obj;
                    try {
                        MReceiver.getInstance().handleStatus(protocolBean);
                    } catch (RemoteException e) {
                        e.printStackTrace();
                    }
                    break;
