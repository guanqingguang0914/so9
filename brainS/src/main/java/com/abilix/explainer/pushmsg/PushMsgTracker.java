   ...
   public Handler getPushMsgHandler(){
    ...
    private class PushMsgHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (data[5] == (byte) 0xF7 && data[6] == (byte) 0x01) {
                ...
                if (data[12] == 0) {
                    ...
                    if (data[13] == 0) {//内置音频
                        ...
                        String soundName = new String(soudNameByte);
                        LogMgr.d("音频名称：" + soundName);
                        if (!TextUtils.isEmpty(soundName) && soundName.startsWith("custom_")) {
                            mPushMsgExcutor.playSound(soundName, 1);
                        } else {
                            mPushMsgExcutor.playSound(soundName + ".mp3", 0);
                        }
                ...
                } else if (data[12] == 4) {//显示自定义图片
                    int dataLenth = data[13];
                    byte[] contentData = new byte[dataLenth];
                    System.arraycopy(data, 14, contentData, 0, dataLenth);
                    String name = new String(contentData);
                    String path = FileUtils.getFilePath(FileUtils.DIR_MEDIA_UPLOAD_IMAGE, name);
                    mPushMsgExcutor.displayImage(path);
                } else if (data[12] == 5) {//显示动画
                    int ImgIndex = data[13];
                    String path = FileUtils.getFilePath(FileUtils.DIR_DEFAULT_IMAGE_ANIM_KU, String.valueOf(ImgIndex), FileUtils._TYPE_GIF);
                    mPushMsgExcutor.displayImage(path);
                }
            ...
            } else if (data[5] == (byte) 0xF7 && data[6] == (byte) 0x0D) {
                int mode = data[12] & 0xFF;
                LogMgr.d("设置人脸识别开关:" + mode);
                mPushMsgExcutor.setFaceTrackMode(mode);
            } else if (data[5] == (byte) 0xF7 && data[6] == (byte) 0x0E) {
                LogMgr.d("获取当前的人脸检测结果");
                mPushMsgExcutor.getFaceTrackedFlag();
            } else if (data[5] == (byte) 0xF7 && data[6] == (byte) 0x0F) {
                int pos = data[12] & 0xFF;
                LogMgr.d("进行人脸追踪:" + pos);
                mPushMsgExcutor.getFaceTrackedPos(pos);
            }
