...
public class RecorderUtils {
    ...
    public synchronized void startRecord(String filePath, int duration, OnCompletionListener listener, boolean updateMicStatus) {
        ...
        final int limited = duration;
        LogMgr.d("startRecord(" + filePath + ", " + duration + ")");

        new Thread(new Runnable() {
            @Override
            public void run() {
                while (mIsRecording) {
                    if (System.currentTimeMillis() - startTime >= limited * 1000) {
                        stopRecord();
                    }
                    try {
                        Thread.sleep(1);
    ...
    private int GET_DB_DURATION = 1000; //ms
    private int CHECK_DB_COUNT = 3;
    ...
    public synchronized float getMicDbValue() {
        ...
        do {
            ...
            if (recordTime > 350 && (recordTime - 9) % 50 == 0 && recordTime != lastTime) {
                lastTime = recordTime;
                if ((int) db > 0) {
                    count++;
                }
                LogMgr.d("recordTime == " + recordTime + " db = " + db + " count = " + count);
            }
        } while (mIsRecording && recordTime < GET_DB_DURATION && count < CHECK_DB_COUNT);
        stopRecord();
        return db;
    }
