...
public class CChartExplainer extends AExplainer {
    ...
    public CChartExplainer(Handler handler) {
        super(handler);
        ...
        do {
            ...
            switch (index) {
                ...
                case 1: // SetMoto
                    ...
                    explainerHelper.setMotorSpeedOpenloop((int) param1, (int) param2);  //这里改为新协议。 一个端口，一个速度。
                    break;
                ...
                case 7: // 7	int getSensorAIValue(int, int)	获取某个端口传感器模拟值
                    //"注：此条协议添加的目的是为了解决传感器值范围不准的问题，Brain端做过滤，再返回；如：超声传感器值50-1500，低于50返回50，高于1500返回1500；
                    /*第一个参数：4字节，int类型，表示端口（1~7）。
                    第二个参数：4字节，int类型，表示传感器类型（1:灰度传感器;2:超声传感器;3:碰撞传感器;4:光敏传感器;5:磁敏传感器;6:火焰传感器;7:温度传感器;）。
                    返回值：传感器模拟值；"*/
                    param1 = explainerHelper.getParam(runBuffer, mValue, 10);
                    int sensorType = (int) explainerHelper.getParam(runBuffer, mValue, 15);
                    ...
                    switch (sensorType) {
                        ...
                        case 0x03: //3:碰撞传感器;
                            ...
                            break;
                        case 0x04: //4:光敏传感器;
                            mValue[reValue] = explainerHelper.getColorForce();
                            break;
                        case 0x05: //5:磁敏传感器;
                        case 0x06: //6:火焰传感器;
                        case 0x07: //7:温度传感器;
                    ...
                    LogMgr.d("====>index:7 reValue::" + reValue + "  " + "ReadAIValue() = " + mValue[reValue]);
                    break;
                ...
                case 8: // AI
                    ...
                    // vjc pad端口号改成1到7
                    mValue[reValue] = explainerHelper.ReadAIValue((int) param1 - 1);
                    LogMgr.d("====>index:8 reValue::" + reValue + "  " + "ReadAIValue() = " + mValue[reValue]);
                    break;
                ...
                case 32:// smallMoto
                    ...
                    int valueBigMotor = (int) ByteUtils.byte2float(runBuffer, 15 + 1);//表示数值
                    int idSetBigMotor = (int) explainerHelper.getParam(runBuffer, mValue, 20);//表示电机端口。0~3（A、B、C、D）
                    int speedBigMotor = (int) ByteUtils.byte2float(runBuffer, 25 + 1);//表示电机速度（-100~100）
                    ...
                    byte[] motor_set_bytes = new byte[4];
                    byte[] motor_value_bytes = new byte[8];
                    byte[] motor_speed_bytes = new byte[4];
                    ...
                    motor_setByte |= (typeBigMotor & 0x0F) << 2;//数值类型(0-速度，1-角度，2-圈数，3-时间，占4位)
                    motor_setByte |= (runBuffer[15] & 0x01) << 1;//数值是否引用变量(占1位)
                    motor_setByte |= runBuffer[25] & 0x01;//速度是否引用变量(占1位)
                ...
                case 70:// setDisplay
                    ...
                    for (int k = 0; k < data_lines; k++) {
                        funPos = explainerHelper.getU16(runBuffer, packetDataLine1Index) * packetDataLength;
                        explainerHelper.readFromFlash(filebuf, funPos, runBuffer, packetDataLength);
                ...
                case 110://设置所有大电机
                    byte[] set_bytes = new byte[4];
                    System.arraycopy(runBuffer, 11, set_bytes, 0, set_bytes.length);
                    byte[] value_bytes = new byte[8];
                    System.arraycopy(runBuffer, 16, value_bytes, 0, 4);
                    System.arraycopy(runBuffer, 21, value_bytes, 4, 4);
                    byte[] speed_bytes = new byte[4];
                    System.arraycopy(runBuffer, 26, speed_bytes, 0, speed_bytes.length);
                    explainerHelper.setAllMotor(set_bytes, value_bytes, speed_bytes, 1, mValue);
                    break;

                case 111://设置所有小电机
                    byte[] sm_set_bytes = new byte[4];
                    System.arraycopy(runBuffer, 11, sm_set_bytes, 0, sm_set_bytes.length);
                    byte[] sm_value_bytes = new byte[8];
                    System.arraycopy(runBuffer, 16, sm_value_bytes, 0, 4);
                    System.arraycopy(runBuffer, 21, sm_value_bytes, 4, 4);
                    byte[] sm_speed_bytes = new byte[4];
                    System.arraycopy(runBuffer, 26, sm_speed_bytes, 0, sm_speed_bytes.length);
                ...
                case 114:
                    ...
                    reValue = explainerHelper.getU16(runBuffer, packetDataValueIndex);
                    mValue[reValue] = explainerHelper.getColorForce();
                    LogMgr.d("getColorForce() = " + mValue[reValue]);
                    break;
                ...
                case 120: //int AI(int) 获取磁敏
                    /*4字节，int类型，表示端口（1~7）*/
                    param1 = explainerHelper.getParam(runBuffer, mValue, 10);
                    explainerHelper.getAIMSensor((int) param1);
                    break;
                ...
                case 121: //void setColorLed(int)	设置彩灯
                    ...
                    explainerHelper.setColorLed(port, mode, frequency, rgb);
                    break;

                case 122: //void setSingleEngine(int,int)	单个舵机控制
                    /*"第一个参数舵机ID号(1-22) 第二个参数舵机角度(-90 - 90)"*/
                    param1 = explainerHelper.getParam(runBuffer, mValue, 10);
                    param2 = explainerHelper.getParam(runBuffer, mValue, 15);
                    explainerHelper.setSingleEngineC5((int) param1, (int) param2);
                    break;

                case 125: //void Scorpion(int)	蝎子控制
                    /*"第一个参数：
                ...
                case 126: //void Tank(int)	坦克（救援车）控制
                    /*"第一个参数：
                    第一字节：命令类型 0x01 移动命令 0x02 功能命令
                ...
                case 131://void setFaceShadowState(int)	设置人脸跟踪状态
                    //第一个参数：0表示关闭功能；1表示开启功能；2关闭预览；3开启预览。
                    int faceMode = (int) explainerHelper.getParam(runBuffer, mValue, 10);
                    LogMgr.d("case 131: onFaceTrack() faceMode = " +  faceMode);
                    explainerHelper.setFaceTrackMode(faceMode);
                    break;

                case 132://int ExamineFace()	人脸检测
                    //无参数。返回值：0：无人脸；1有人脸。
                    reValue = explainerHelper.getU16(runBuffer, packetDataValueIndex);
                    mValue[reValue] = explainerHelper.getFaceTrackedFlag();
                    LogMgr.d("case 132:人脸检测--> " + mValue[reValue]);
                    break;

                case 133://int Faceshadow(int)	人脸跟踪
                    //第一个参数：0表示获取人脸在X轴坐标位置；1表示获取人脸在Y轴坐标位置；返回值：获取到的人脸当前轴位置（在二维坐标系中）
                    int posXY = (int) explainerHelper.getParam(runBuffer, mValue, 10);
                    reValue = explainerHelper.getU16(runBuffer, packetDataValueIndex);
                    mValue[reValue] = explainerHelper.getFaceTrackedPos(posXY);
                    LogMgr.d("case 133:人脸跟踪[" + posXY + "]--> " + mValue[reValue]);
                    break;

                case 134://void setDisplayImage(char*)	显示图片
                    //"第一个字节：类型 0：拍照图片；1：自定义图片(在多媒体目录下/Abilix/media/upload/image/)char*类型，表示图片名称（N<19 字节）"
                    int displayType = runBuffer[10] & 0xFF;
                    switch (displayType) {
                        case 0x00:
                            param1 = explainerHelper.getParam(runBuffer, mValue, 10);
                            LogMgr.d("case 134:显示图片[" + displayType + "]--> " + param1);
                        ...
                        case 0x01:
                            ...
                            LogMgr.d("case 134:显示图片[" + displayType + "]--> " + picName);
    ...
    public void stopExplain() {
        ...
        explainerHelper.setFaceTrackMode(0);
    }
