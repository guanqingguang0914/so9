...
public class UsbCamera implements IUsbCamera {
    ...
    //add end
    private boolean allowDestroy = true;  //xiongxin@20171214 add
    ...
    public void startFaceTrackAct(Context context,FaceTrackCallback cb){
        ...
        mContext = context;
        try {
            Intent intent = new Intent("com.abilix.learn.rtspserver.FaceTrackService.action");
    ...
    public void preViewFaceTrack(boolean show){
        int result = show?1:0;

        try {
            Intent intent = new Intent("com.abilix.learn.rtspserver.FaceTrackService.action");
            intent.setPackage("com.abilix.learn.rtspserver");
            intent.putExtra("openprewindwon",result);
            mContext.startService(intent);
        }catch (Exception e){
            Toast.makeText(mContext,"moudle not found",Toast.LENGTH_SHORT).show();
        }
    }

    private void registerFaceTrackCallbackEvent(Context context){
        IntentFilter filter = new IntentFilter();
        filter.addAction("com.abilix.learn.rtspserver.facetrack.finish.action");
        filter.addAction("com.abilix.learn.rtspserver.facetrack.callback.action");
        context.registerReceiver(faceTrackreceiver,filter);
    }
    ...
    private BroadcastReceiver faceTrackreceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            LogMgr.i("收到人脸识别数据");
            String action  = intent.getAction();
            if (action.equals("com.abilix.learn.rtspserver.facetrack.finish.action")){
                if (mFaceTrackCallback !=null) mFaceTrackCallback.onFinish();

            }else if (action.equals("com.abilix.learn.rtspserver.facetrack.callback.action")){
    ...
    private synchronized void destoryCameraClient() {
        synchronized (mCameraClientLock) {
            if (mCameraClient != null && allowDestroy) {
    ...
    public synchronized void setBrightnessS(final Context context, final int cmd, final CameraStateCallBack cameraStateCallBack) {
        ...
        mUsbCameraThread.runOnUsbCameraThread(new Runnable() {
            @Override
            public void run() {
                allowDestroy =false; //xiongxin@20171214 add
                mContext = context;
                if (getCameraDevice() != null) {
                    initCameraClient(context);
                    synchronized (mLock) {
                    ...
                    allowDestroy =true; //xiongxin@20171214 add
                }
            }
        });
