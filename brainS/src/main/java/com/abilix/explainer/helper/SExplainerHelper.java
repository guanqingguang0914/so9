...
public class SExplainerHelper extends AExplainHelper {
    ...
    public void setSingleMotorInWheelMode(int id, int speed) {
        ...
        //runServoS5InWheelMode(new ServoWheelModeSet(id, speed));
    }

    public void setDoubleMotorInWheelMode(int id01, int speed01, int id02, int speed02) {
        ...
        if (speed01 + speed02 == 0) {
            if (speed01 == 0) {
                ...
                setServoWalkStraightLine(id01, id02, speed01);
            }
        } else {
            setSingleMotorInWheelMode(id01, speed01);
            setSingleMotorInWheelMode(id02, speed02);
        }

        //runServoS5InWheelMode(
        //        new ServoWheelModeSet(id01, speed01),
        //        new ServoWheelModeSet(id02, speed02));
    }
    ...
    /**
     * 车轮模式转动
     * @param sets ServoWheelModeSet对象
     */
    private void runServoS5InWheelMode(ServoWheelModeSet... sets) {
        int length = sets.length;
        byte[] data = new byte[1 + 1 + 2 * length];

        data[0] = (byte) (1 + 2 * length);
        data[1] = (byte) length;

        for (int i = 0; i < sets.length; i++) {
            ServoWheelModeSet set = sets[i];

            data[2 + i * 2] = (byte) set.id;
            data[3 + i * 2] = (byte) (set.speed & 0xFF);
        }

        ProtocolUtils.write((byte) 0x05, (byte) 0xA3, (byte) 0xB7, data);
    }
    ...
    public void initGyroscope() {
        ProtocolUtils.sendPatchCmdToControl(34, new byte[]{0x00});
    }
     ...
     * @param speed 速度
     */
    public void setServoWalkStraightLine(int id01, int id02, int speed) {
        byte[] data = new byte[6];
        byte[] speedBytes = ByteUtils.intTo4Bytes(speed);
        System.arraycopy(speedBytes, 0, data, 0, 4);
        data[4] = (byte) id01;
        data[5] = (byte) id02;
        ProtocolUtils.sendPatchCmdToControl(35, data);
    }
     ...
     * @param speed 速度
     */
    public void setServoWalkRound(int id01, int id02, int speed) {
        byte[] data = new byte[6];
        byte[] speedBytes = ByteUtils.intTo4Bytes(speed);
        System.arraycopy(speedBytes, 0, data, 0, 4);
        data[4] = (byte) id01;
        data[5] = (byte) id02;
        ProtocolUtils.sendPatchCmdToControl(36, data);
    }

    /**
     * 设置双电机停止运动
     */
    public void setServoWalkStop() {
        ProtocolUtils.sendPatchCmdToControl(37, new byte[]{0x00});
    }

    /**
     * 退出双电机运动校准模式
     */
    public void setServoWalkExit() {
        ProtocolUtils.sendPatchCmdToControl(38, new byte[]{0x00});
    }

    //车轮模式舵机设置
    class ServoWheelModeSet {
        private int id;
        private int speed;

        public ServoWheelModeSet(int id, int speed) {
            this.id = id;
            this.speed = (100 * speed) / 1023 + 100;
        }
    }
