...
public class HExplainerHelper extends AExplainHelper implements IHRobot {
    ...
    private void controlServo(int id, int type) {
        ...
        if (id == 0) {
            if (type == 3) {
                int type0 = GlobalConfig.BRAIN_TYPE == GlobalConfig.ROBOT_TYPE_H3 ? 2 : 3;
                ProtocolUtils.sendProtocol((byte) GlobalConfig.BRAIN_TYPE, (byte) 0xA3, (byte) 0x78, new byte[]{2, (byte) type0, 0});//按照H的协议来。
            } else {
                ...
                ProtocolUtils.sendProtocol((byte) GlobalConfig.BRAIN_TYPE, (byte) 0xA3, (byte) 0x65, dataALL);//按照H的协议来。
            }
        } else {
            if (type == 3) {
                runServo(id, 200, 0);
