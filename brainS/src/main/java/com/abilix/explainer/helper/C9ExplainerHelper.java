...
public class C9ExplainerHelper extends AExplainHelper {
    ...
    public void readandsend(int num) {
        ...
        if (num == 1) {
            ...
            if (temp.length >= 7 && temp2.length >= 7) {

                int[] lineVaues = new int[7];
                int[] bgValues = new int[7];
                int[] avgValues = new int[7];

                for (int i = 0; i < 7; i++) {
                    lineVaues[i] = Integer.parseInt(temp[i]);
                    bgValues[i] = Integer.parseInt(temp2[i]);
                    avgValues[i] = (lineVaues[i] + bgValues[i]) / 2;
                }

                writeENVToEEPROM(lineVaues, bgValues, avgValues);

                LogMgr.e("AI:" + Arrays.toString(avgValues));
                environment(avgValues[0], avgValues[1], avgValues[2], avgValues[3], avgValues[4], avgValues[5], avgValues[6]);
            } else {
                LogMgr.e("readandsend() Error: temp.length = " + temp.length + " temp2.length = " + temp2.length);
                environment(1000, 1000, 1000, 1000, 1000, 1000, 1000);
            }
        } else {
            // 这里是5灰度的值。
            ...
            if (temp.length >= 5 && temp2.length >= 5) {

                int[] lineVaues = new int[7];
                int[] bgValues = new int[7];
                int[] avgValues = new int[7];

                lineVaues[0] = Integer.parseInt(temp[0]);
                lineVaues[1] = 0;
                lineVaues[2] = Integer.parseInt(temp[1]);
                lineVaues[3] = Integer.parseInt(temp[2]);
                lineVaues[4] = Integer.parseInt(temp[3]);
                lineVaues[5] = 0;
                lineVaues[6] = Integer.parseInt(temp[4]);

                bgValues[0] = Integer.parseInt(temp2[0]);
                bgValues[1] = 0;
                bgValues[2] = Integer.parseInt(temp2[1]);
                bgValues[3] = Integer.parseInt(temp2[2]);
                bgValues[4] = Integer.parseInt(temp2[3]);
                bgValues[5] = 0;
                bgValues[6] = Integer.parseInt(temp2[4]);
    ...
    public void writeENVToEEPROM(int[] lineValues, int[]bgValues, int[] avgValues) {
        try {
            byte[] readBuff = ProtocolUtils.sendProtocol((byte) 0x01, (byte) 0xA4, (byte) 0x57, null);
            if (readBuff[0] == (byte) 0xAA && readBuff[1] == (byte) 0x55 &&
                    readBuff[5] == (byte) 0xF0 && readBuff[6] == (byte) 0xB8) {
                byte[] data = new byte[129];
                int offset = 1;
                data[0] = 0x01;
                System.arraycopy(readBuff, 11, data, 1, 128);
                for (int i = 0; i < 7; i++) {
                    data[offset + i * 2] = (byte) (lineValues[i] >> 0x08 & 0xFF);
                    data[offset + i * 2 + 1] = (byte) (lineValues[i] & 0xFF);

                    data[offset + (i + 7) * 2] = (byte) (bgValues[i] >> 0x08 & 0xFF);
                    data[offset + (i + 7) * 2 + 1] = (byte) (bgValues[i] & 0xFF);

                    data[offset + (i + 7 * 2) * 2] = (byte) (avgValues[i] >> 0x08 & 0xFF);
                    data[offset + (i + 7 * 2) * 2 + 1] = (byte) (avgValues[i] & 0xFF);
                }
                ProtocolUtils.sendProtocol((byte) 0x01, (byte) 0xA4, (byte) 0x56, data, 500);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public int[] readENVToEEPROM() {
        ...
        try {
            ...
            if (readBuff[0] == (byte) 0xAA && readBuff[1] == (byte) 0x55 &&
                ...
                System.arraycopy(readBuff, 11, data, 0, 128);
                for (int i = 0; i < 128; i += 4) {
                    values[i / 2] = ((data[i + 3] & 0xFF) << 8) | (data[i + 2] & 0xFF);
                    values[i / 2 + 1] = ((data[i + 1] & 0xFF) << 8) | (data[i] & 0xFF);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public int distance(int port) {
        ...
        if (port != 0) {
            ...
            int tryCount = 0;
            while (!plugged && tryCount < 5) {
                if (port == getPortByAIType(AI_TYPE_ULTRASONIC)) {
                    plugged = true;
                }
                tryCount++;
            }
